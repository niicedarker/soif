...
async function startBotWithPhone(phoneNumber) {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (statusCode === 403 || errorMessage.includes("banned") || errorMessage.includes("Stream Errored (custom)")) {
        latestPairingCode = "🚨 ALERTE CRITIQUE : Ce numéro WhatsApp est banni par Meta/WhatsApp !";
  ...
  }, 1 * 1000); // Intervalle sécurisé à 30 secondes pour éviter le spam/bannissement
}
